...
fn read_concentrations(time_step: usize) -> Vec<Float> {
    let dir = Path::new(crate::io::DATA_PATH).join(time_step.to_string());
    let prefix = "concentration";
...
fn append_passive_scalar_vtk<P: AsRef<Path>>(
    ...
    writeln!(file, "SCALARS concentration float 1")?;
...
fn passive_scalar_vtk(config: &Config) {
    crate::io::collect_time_steps()
        ...
        .for_each(|time_step| {
            ...
            let concentrations = read_concentrations(time_step);
            println!(
                "Writing {} for time step {}.\n",
                file_name.bold().yellow(),
                time_step.to_string().bold().yellow()
            );
            if let Err(e) = append_passive_scalar_vtk(&path, config, &concentrations) {
